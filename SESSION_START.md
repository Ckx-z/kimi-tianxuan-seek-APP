...
- **阶段**：阶段 12 已完成 —— 双模型路由上线（D22）；指纹/校准路线证伪关闭；Hadamard 交互确认保留
- **可运行**：双击 `start_app.vbs`（推荐，静默启动）或 `.venv/Scripts/python.exe app/gradio_app.py`
- **模型（路由模式）**：醛/胺都已见 → `tree_v4`（144 特征含 TE，LOGO 0.8784）；任一未见 → `tree_v4_noTE`（142 特征，双留出 3 种子均值 0.6824 最强）；路由键 `models/monomer_pool.json`（醛 896/胺 1366）；`tree_v3` 为单模型模式默认与回退；GNN v5.3 并行输出不变
- **评估协议**：LOGO 与醛胺双留出同时报告；双留出必须报多分组种子（单种子噪声 ±0.03）；路由验证见 `reports/routing_eval.json`
- **前端**：「打分理由」SHAP 归因跟随路由模型（热态 ~0.04s）；预测标注实际模型 + 路由原因
- **最新报告**：`EXPERIMENTS/exp_006.md`（指纹证伪）、`exp_007.md`（交互消融 + fold3 诊断）、`exp_008.md`（路由评估）
- **记忆系统**：.agents/ 目录已创建，AGENTS.md + session_index + session_state 已建立
- **日报双轨制**：AI 日报 + 人日报，每天工作结束后生成
- **已知阻塞**：无；真正瓶颈是双未见单体泛化（0.63-0.68，指纹已证伪 → GNN 表征迁移方向）
