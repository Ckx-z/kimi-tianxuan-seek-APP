...
check_hard_constraints() 支持 mode 参数：
  - mode="full": 使用全部 23 条违反规则（向后兼容）。
  - mode="core": 仅使用用户确认的核心规则（官能团有效性 + 对称/对位）。

get_rule_vector() 支持 reduced 参数：
  - reduced=False: 返回完整 34 维规则向量（23 违反 + 11 F/CF3）。
  - reduced=True:  返回精简规则向量（核心违反 + 11 F/CF3），
                    降低模型对化学规则的依赖，让模型自己学习成膜模式。

核心规则（mode="core" / reduced=True 时保留）:
  单体级:
    - 官能团有效性：无有效醛基、无有效伯胺、醛/胺基数不匹配
    - 官能团非中心对称
    - 对位违规：非对位(间位/邻位)、C3 邻位/对位

完整规则列表（mode="full" 时额外保留，但不建议作为硬性过滤）:
  - 无芳香原子、直链分子、非平面环、支链>4碳、含硼酸酯
  - 自反应、拓扑不匹配、对位苯链过长、无苯环长支链>8
  - 芳环过少/过多、双刚、超尺寸、大小失配
"""
...
def check_hard_constraints(ald_smi: str, am_smi: str,
                            n_ald_claimed: int = 0,
                            n_am_claimed: int = 0,
                            mode: str = "full") -> Tuple[List[str], float, str]:
    """检查硬约束。

    Args:
        ald_smi: 醛 SMILES
        am_smi: 胺 SMILES
        n_ald_claimed: 声称的醛基数
        n_am_claimed: 声称的胺基数
        mode: "full" 使用全部 23 条规则；"core" 仅使用用户确认的核心规则
              （官能团有效性 + 对称/对位）。

    Returns:
        (violations, bonus, bonus_reason)
    """
    ald_mol = Chem.MolFromSmiles(ald_smi)
    am_mol = Chem.MolFromSmiles(am_smi)
    if ald_mol is None or am_mol is None:
    ...
    if mode == "core":
        # 仅保留核心规则（官能团有效性 + 对称/对位）
        def _is_core(violation: str) -> bool:
            v_base = violation.split("(")[0] if "(" in violation else violation
            return any(core in violation or v_base in core for core in CORE_RULE_NAMES)
        unique = [v for v in unique if _is_core(v)]
...
# 核心规则：用户确认必须保留为硬性约束的规则（官能团有效性 + 对称/对位）
CORE_RULE_NAMES = [
    "无有效醛基",
    "无有效伯胺",
    "醛基数不匹配",
...
CORE_VIOLATION_DIM = len(CORE_RULE_NAMES)

# 精简版规则向量维度：核心违反 (9) + F/CF3 描述符 (11) = 20
REDUCED_RULE_DIM = CORE_VIOLATION_DIM + 11


def get_rule_vector(ald_smi: str, am_smi: str, reduced: bool = False) -> dict[str, float]:
    """返回规则特征向量（字典形式，键为可解释名称）。

    Args:
        ald_smi: 醛 SMILES
        am_smi: 胺 SMILES
        reduced: 是否返回精简版（仅核心违反 + F/CF3 描述符）。
                 精简版约 20 维；完整版约 34 维。
    ...
    if reduced:
        violation_names = CORE_RULE_NAMES
        f_names = RULE_NAMES[-11:]  # F/CF3 描述符名称
    else:
        violation_names = RULE_NAMES[:23]
        f_names = RULE_NAMES[-11:]

    vec = {name: 0.0 for name in violation_names + f_names}

    if ald_mol is None or am_mol is None:
        return vec

    violations, _, _ = check_hard_constraints(ald_smi, am_smi, mode="core" if reduced else "full")
    ...
    for name, val in zip(f_names, f_cf3_features):
        vec[name] = val
