...
_PROMPT = (
    ...
    "只有定性结论（如「SEM 显示连续致密膜」）时也输出该 characterization 条，"
    "把结论写进 conclusion（metrics 可留空）；\n"
    "   technique 必须用规范写法之一：PXRD / FTIR / BET / SEM / TEM / AFM / "
    "PL / UVVis / NMR / TGA / XPS / contact_angle / separation_flux / "
    "separation_selectivity / mechanical / photocatalysis / electrochem / "
    "dft（XRD 写成 PXRD、IR 写成 FTIR、UV-Vis 写成 UVVis）；\n"
    "4) 同一组实验的 condition/characterization/property 都挂同一 group_id；"
    "5) 成膜结论必须给出（含不成膜/失败体系）；\n"
    "6) **SMILES 纪律**：monomer_pair / film_outcome / monomer 必须给出原文"
    "明确出现的 SMILES（monomer 用 monomer_smiles）；原文/图中没有给出结构式时"
    "**不要输出**这些 kind，改输出 condition / characterization / property / "
    "conclusion 等不依赖 SMILES 的条目。禁止用空字符串占位。\n\n文献片段：\n"
)
