...
> 最后更新：2026-07-21（阶段 13 阶段 A：GNN embedding 迁移过门槛——pair_emb 双留出 0.6824→0.844，立项阶段 B 逐折重训）
...
| **阶段 13** | **GNN embedding 迁移：阶段 A 可行性（过门槛）→ 阶段 B 逐折重训（待做）** | 🔄 **进行中** |
...
**阶段 B（待做，第一优先）**：逐折重训 GNN 排除记忆泄漏——每个双留出折只用训练折数据重训 GNN → 提取 embedding → 同口径评估。先 1 种子 5 折摸底训练成本，再 3 种子全量。确认后：接入路由两臂重测（D23 遗留两协议冲突同口径复测）+ App 预测时 embedding 提取方案。

### 下一步

**阶段 13 后续（2026-07-21 刷新）：**

1. **阶段 B：逐折重训 GNN（第一优先）**：调研旧项目训练管线（`scripts/train_v4.py` / v5 训练脚本），实现"折内训练 GNN → 提取 embedding → 树模型评估"闭环；1 种子 5 折摸底 → 3 种子全量。判定：收益保住 → 路由两臂接入重测 + App 集成；收益消失 → 路线关闭，转外部数据预训练/配对级手工表征。
2. **fold3 型"区域漂移大单体"专项切片**：表征升级评估时对 raw-feature 包络外样本单独计分（逐折标配已就位；exp_009 显示 pair_emb 使 fold3 0.364→0.65+，阶段 B 确认后重点复测）。
3. **更早的候选（仍有效）**：异常检测再评估（实验 D 条件泄漏 0.9112）、SMILES 清洗诊断、LightGBM 对比、仅二聚体 3D 消融（D19 复盘点）。

> 已关闭：routed_strict 切换评审（D23 已切换）、双留出逐折报告标配化（D23 已落地）、Morgan/MACCS 指纹入模（exp_006 证伪）、概率校准（证伪）、**mono_emb 单体级 embedding（exp_009 证伪）**。
...
- ~~双未见单体泛化的表征升级路线~~ **阶段 A 已通关（D24，2026-07-21）**：pair_emb +0.16 过门槛 → 阶段 B 逐折重训 GNN 排除记忆泄漏后定真伪；mono_emb 证伪关闭
- 双未见场景 v4 vs noTE 两协议方向冲突（双留出网格 noTE 胜、LOGO 胺未见桶 v4 胜，均在噪声量级）：表征升级后两协议同时复测（D24 配套）
