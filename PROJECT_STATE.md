...
> 最后更新：2026-08-31（阶段 33：**v1.3.0 发布**——DFT 全局任务进度与完成通知（悬浮徽标 + 5s 轮询 + Notification）+ 学术紫金精修版主题（方向 A：柔和紫/暖金/语义三色/等宽数据数字）；792 passed）
...
- **Electron 桌面应用**：当前版本 **v1.3.0**，NSIS 安装包（约 445MB），GitHub Release 分发（exe + zip + latest.yml），**自动更新链路 v0.2.5 起端到端武装；v1.0.1 起版本变更自动清 Chromium 缓存（升级不再见旧界面）**
