...
> 最后更新：2026-07-20（阶段 12 收官：路由切换 routed_strict——仅双未见走 tree_v4_noTE（D23）；双留出逐折报告标配化）
...
**阶段 13 候选（源自阶段 12 遗留，2026-07-20 收官刷新）：**

1. **双未见单体泛化（真正瓶颈，未解决，明日第一优先）**：双留出 0.63-0.68；指纹已证伪，剩余方向：(a) **GNN 表征迁移**（embedding 蒸馏/迁移入树模型，而非手工位）——先做可行性评估；(b) 配对/界面级表征；(c) 外部数据预训练。
2. **fold3 型"区域漂移大单体"专项切片**：表征升级评估时对 raw-feature 包络外样本单独计分（逐折标配已就位，切片可挂在 fold_summary 最难折上）。
3. **更早的候选（仍有效）**：异常检测再评估（实验 D 条件泄漏 0.9112）、SMILES 清洗诊断、LightGBM 对比、仅二聚体 3D 消融（D19 复盘点）。

> 已关闭：routed_strict 切换评审（D23 已切换）、双留出逐折报告标配化（D23 已落地）、Morgan/MACCS 指纹入模（exp_006 证伪）、概率校准（证伪）。
