...
> 最后更新：2026-07-15（模型 v2 训练验证完成，准备接入 3D/二聚体描述符）
...
**按用户确认的“分两步走”方案，进入阶段 9：**

1. **接入单体 3D 描述符**：在 `src/features/descriptors.py` 中调用 `src/features/chemistry/conformer.py`
2. **接入二聚体 3D 描述符**：调用 `src/features/chemistry/dimer.py`，构建醛-胺对的几何/能量特征
3. **对比实验**：与当前 v2 做留一单体 CV 对比，量化 3D/二聚体描述符的收益
4. **消融实验**：分别关闭 3D、二聚体、交互特征，分析各自贡献
5. **异常检测再评估**：条件特征实验 D PR-AUC 0.9112 异常高，需单独诊断是否与标签泄漏有关
