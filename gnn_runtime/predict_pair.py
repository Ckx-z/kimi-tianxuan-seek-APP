...
def main():
    ...
    use_global = ckpt.get("use_global", False)   # v5.4：全局虚拟节点
    if use_3d:
        cfg["model"]["use_3d"] = True
    cfg["model"]["use_rules"] = use_rules
    cfg["model"]["use_global"] = use_global
    ...
    # v5.4：Isotonic 校准器（与 checkpoint 同目录 calibrator.pkl，可选）
    calibrator = None
    cal_path = os.path.join(os.path.dirname(os.path.abspath(args.model)),
                            "calibrator.pkl")
    ...
    # ── 3. 构建分子图 ──
    ald_graph = smiles_to_graph(args.ald, role=0, with_global=use_global)
    amine_graph = smiles_to_graph(args.amine, role=1, with_global=use_global)
    ...
    # v5.4：Isotonic 校准（输出用校准分；MC std 保持原始口径）
    prob_cal = float(calibrator.predict([prob_mean])[0]) \
        if calibrator is not None else prob_mean
    ...
    print(f"  {'─' * 40}")
    print(f"  成膜概率  : {prob_cal:.4f}")
    if calibrator is not None:
        print(f"  原始概率  : {prob_mean:.4f}（校准前）")
