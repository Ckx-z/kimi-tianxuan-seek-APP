...
## 〇之三、已实施：第 3 批（首页仪表盘化 + 统一页头 + 动效，2026-09-12）

| 改动 | 文件 | 说明 |
|------|------|------|
| **首页仪表盘化** | `components/home/KpiCard.tsx`（新）、`components/home/Sparkline.tsx`（新）、`pages/Home.tsx` | KPI 卡：标签弱化 + **数值成为焦点**（28px 等宽数字 + 弱化单位）+ 图标收进柔和色块 + 底部「真实环比 + 迷你趋势」。**只画真实数据**：实验记录数用「近 8 周新增」折线（由 experiment_date 派生，含环比 +N），建议数用「各批次条数」柱状；收藏/方案无时间序列则给提示文案，**不造数** |
| **统一页头** | `components/layout/PageHeader.tsx`（新） | 标题（衬线学术签名）+ 一句话副标题 + 右侧主行动区，间距进入 8pt 节奏；已应用于首页 / 实验记录 / 我的（其余页面外观不变，可后续平滑迁移） |
| **动效** | `index.css`、`AppLayout.tsx` | 路由切换 `page-enter`（220ms 淡入上移，按 pathname key 重放）；列表 `stagger` 依次落位；可交互卡 `card-interactive`（hover 抬升 + 阴影）；**尊重 `prefers-reduced-motion`** |
| 表格细节 | `components/ui/table.tsx` | 表头加弱底色（不再只靠一条边框）、分隔线更淡；表格数字全站等宽 |
| 加载/空态 | `pages/Home.tsx` | 载入中用 `Skeleton`（原来是「—」占位）；空态换成 `Empty`（图标 + 标题 + 一句引导，而不是干巴巴一行字） |

**验收**：`tsc -b` + `vite build` 通过；同后端真实数据截图（首页/实验记录/我的）；
安装版热更新（`index.html` 哈希一致、8 文件、health 1.9.3）并实拍确认仪表盘生效。

**回退脚本顺带修缺陷**：快照列表原按**名称**排序 → `ui-batch3` 会排在 `ui-overhaul1` 之后，
导致「默认取最近一个快照」取错版本；已改为按 `LastWriteTime` 倒序（并验证顺序正确）。
另注意：**编辑该 .ps1 后必须重新保存为 UTF-8 BOM**（编辑工具重写会丢 BOM，PS 5.1 会按 ANSI
解析、中文串报语法错）——本轮已再次踩到并修正。

---

## 〇之四、第 4 批（剩余页头统一 + 深色复核）与**主题状态 bug 修复**（2026-09-12）

### A. bug 修复（用户报：「来回切换主题和浅色深色模式会导致主题和选择的主题不匹配」）

**复现**（安装版实测，修改前）：
```
起始 purple-gold → 设置页选「暖纸松石」→ data-theme=warm-paper ✓
→ 侧栏切一次明暗 → data-theme 被打回 purple-gold ✗（设置页仍显示选中暖纸松石）
```

**根因**：`useTheme()` 被 **2 处独立调用**（`pages/Settings.tsx` 配色主题卡、
`components/layout/AppLayout.tsx` 侧栏明暗开关），各自持有独立 `useState`，
却都往同一批 localStorage key 与 `<html data-theme>` / `.dark` 写 —— 侧栏那份
**过期副本**在任何一次重渲染（如切明暗）时把旧主题写回，覆盖设置页的新选择。

**修复**：新增 `hooks/theme-store.ts` —— 模块级**单一数据源** + `useSyncExternalStore`，
所有调用方共享同一份状态，任何变更只应用一次（DOM + localStorage 只写一处）；
`hooks/use-theme.ts` 保留原 API 作兼容层；首次加载立即落 DOM（避免首帧闪主题）。
验证：**6 轮「切主题 ↔ 切明暗」交替压力测试**，`data-theme` / `cof-scheme` / 设置页选中态
三者始终一致（开发后端 + **安装版**各测一遍，均 6/6 通过）。

### B. 第 4 批 UI

| 改动 | 文件 | 说明 |
|------|------|------|
| 页头统一收尾 | `Query/Batch/Dft/Iterate/Settings` | 五个页面迁移到 `PageHeader`（标题 + 一句话副标题 + 8pt 节奏），至此**九个页面全部统一** |
| 深色模式复核 | —— | 用 localStorage 精确控制（`cof-theme` / `cof-scheme`，修 store 后不再被注入覆盖）复核「暖纸+深色」「紫金+深色」的首页与密集页：KPI 色块/迷你图/卡片描边/选中态对比度均正常，无缺陷需修 |
| 图标尺寸 | —— | 统计后确认已较统一（`h-4` 40 处为主，小号 18 处、大号 4 处），**不做批量改动**以免无意义 churn |

**回退**：`ui-batch4-20260912-202641`（含本批 + 主题修复）；`baseline-*` 仍可一键回到最初。
