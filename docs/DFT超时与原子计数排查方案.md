...
> 状态：**诊断完成，修复已实施（本地提交 b1b9275/e95c342，未推送）**。
> 以下保留诊断原文，文末附实施结果与真实基准。

---

## 实施结果（2026-09-01，真实基准验证）

| 配置（93 原子 pair，ωB97X-D3BJ/def2-SV(P) quick 档，24 线程） | 实测耗时 | 结果 |
|---|---|---|
| 仅结合能（with_props=false，CP 3 次 SCF） | **1532s ≈ 25.5 分钟** | e_bind=-4.787 kJ/mol ✅ |
| 完整性质（with_props=true，6 次 SCF） | **2102s ≈ 35.0 分钟** | gap=7.93 eV、偶极=2.74 D、fchk ✅ |

- 两种模式均在用户要求的 **40 分钟**内；两模式 e_bind 完全一致（-4.787），
  验证 with_props 实现正确。
- 修复前（4 线程 + 高精度档 + 5400s 上限）实测超时失败；修复后 ≈9× 提速。
- 已实施：线程默认 24（前端可选 1–32）、超时 >90 原子 10800s、仅结合能模式、
  失败现场保留（psi4_output.dat/result.json 路径附错误信息）、运行卡已运行
  时长、>90 原子 quick 档建议、缓存 key 区分 with_props（|noprops）。
- 回归：pytest 823 passed / 1 skipped；tsc + vite build 通过。
- 40 分钟配置建议：quick 档 + 24 线程即可（要最快可加勾「仅结合能」）；
  高精度档（def2-SVP）同体系预计 60–90 分钟，界面会自动提示换 quick 档。
