...
- **最新版本**：**v1.6.1** → 📥 [GitHub Release 下载页](https://github.com/Ckx-z/kimi-tianxuan-seek-APP/releases/latest)
...
- **核心能力**：成膜概率预测（tree 模型内置必可用，**GNN v5.4 随包内置**——装好 dphuanjing 推理环境即可启用，未装自动降级）+ 打分理由 + OOD 三级判定 + 成网红线 + 批量排序 + **DFT 计算（内置 xTB 引擎，二聚体结合能）** + 收藏夹 + 实验记录（草稿/时间线/附件/复盘字段/导出 Word）+ LLM 实验方案卡 + GraphRAG 方案迭代（基于用户实验数据闭环）+ 科研助手 Agent（对话式，9 工具读写闭环 + 记忆编译）+ **科研助手深度研究（v1.6.0：联网搜索 / 学术检索 arXiv·PubMed·Semantic Scholar·Crossref / 网页全文阅读 / 计划-检索-批判-报告循环，带 DOI 引用与 Word 导出）+ 引用核验 + 技能方法论 + 按单体组记忆** + 文献体系（收藏/引用带真实 DOI 链接，支持 DOI/标题/**PDF 上传**三种录入方式，LLM 提取元数据 + 人工审核入库）
- **致谢**：本项目（含本次 v1.6.1 更新）由 DeepSeek 协助完成。
