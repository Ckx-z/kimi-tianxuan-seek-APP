...
- **最新版本**：**v1.8.0** → 📥 [GitHub Release 下载页](https://github.com/Ckx-z/kimi-tianxuan-seek-APP/releases/latest)
...
- **核心能力**：成膜概率预测（tree 模型内置必可用，**GNN 随包内置并支持文献/实验反馈微调**——装好 dphuanjing 推理环境即可启用，未装自动降级）+ 打分理由 + OOD 三级判定 + 成网红线 + 批量排序 + **DFT 计算（内置 xTB 引擎，二聚体结合能）** + 收藏夹 + 实验记录（草稿/时间线/附件/复盘字段/导出 Word）+ LLM 实验方案卡 + GraphRAG 方案迭代（基于用户实验数据闭环）+ 科研助手 Agent（对话式，9 工具读写闭环 + 记忆编译，会话新建即保存/删除 + 一对话一报告）+ **科研助手深度研究（联网搜索 / 学术检索 arXiv·PubMed·Semantic Scholar·Crossref / 网页全文阅读 / 计划-检索-批判-报告循环，带 DOI 引用与 Word 导出）+ 引用核验 + 技能方法论 + 按单体组记忆** + 文献体系（收藏/引用带真实 DOI 链接，支持 DOI/标题/**PDF 上传**三种录入方式，LLM 提取元数据 + 人工审核入库，**文献图谱：结构图/光谱/机理图上传与画廊、SMILES 一键生成结构式并可导入打分验证**）+ **GNN 模型演进（v1.8.0：打分页「反馈打分不合理」+ 设置页反馈队列/文献 PDF·实验 CSV 导入/冻结底层微调重训/金标准验证闸门/版本激活回退对比——只修正 GNN，树模型作为不动基准）**
- **致谢**：本项目（含本次 v1.8.0 更新）由 DeepSeek 协助完成。

---

## DFT 计算模块说明（工具箱 → DFT 计算）

**算什么**：两单体（醛 + 胺）先经亚胺缩合生成**二聚体**（程序自动生成，无需 CAS 号），再计算二聚体与第三物质的结合能：

$$E_{bind} = E(\text{二聚体}\cdot X) - E(\text{二聚体}) - E(X)$$

第三物质 X 可选：**自身二聚体**（π-π 堆积/自聚集倾向，对应结晶成膜驱动力，默认）、**溶剂分子**（成膜环境筛选）、**另一组二聚体**（异质堆积）、**自定义分子**（SMILES 或内置画板绘制）。

**用什么方法**：内置 [xTB](https://github.com/grimme-lab/xtb)（Grimme 课题组，[DOI: 10.1002/wcms.1493](https://doi.org/10.1002/wcms.1493)）半经验紧束缚引擎，双档位：

| 档位 | 方法 | 速度 | 适用 |
|---|---|---|---|
| 精确（默认） | GFN2-xTB 几何优化 | 数十秒~几分钟 | 正式比较 |
| 快速 | GFN-FF 力场 | 秒级 | 大批量初筛 |
...
```bash
E:\ANACONDA\python.exe -m pytest tests/ -v    # 基线 578 项
```
