...
- **最新版本**：**v1.9.0** → 📥 [GitHub Release 下载页](https://github.com/Ckx-z/kimi-tianxuan-seek-APP/releases/latest)
...
- **核心能力**：成膜概率预测（tree 模型内置必可用，**GNN 随包内置并支持文献/实验反馈微调**——装好 dphuanjing 推理环境即可启用，未装自动降级）+ 打分理由 + OOD 三级判定 + 成网红线 + 批量排序 + **DFT 计算（内置 xTB 引擎，二聚体结合能）** + 收藏夹 + 实验记录（草稿/时间线/附件/复盘字段/导出 Word）+ LLM 实验方案卡 + GraphRAG 方案迭代（基于用户实验数据闭环）+ 科研助手 Agent（对话式，9 工具读写闭环 + 记忆编译，会话新建即保存/删除 + 一对话一报告，深度研究问答持久化）+ **科研助手深度研究（联网搜索 / 学术检索 / 网页全文阅读 / 计划-检索-批判-报告循环，带 DOI 引用与 Word 导出）+ 引用核验 + 技能方法论 + 按单体组记忆** + **科研知识库（v1.9.0：文献录入+图谱统一，LLM 全维度结构化提取——单体/成膜体系/条件/15 类表征含分离选择性与 DFT/性能/机理/负样本，按实验组分组审核，自动入知识图谱侧车图并本地向量化，跨文献数值检索，条目一键打分验证/DFT 对照/GNN 反馈）+ 文献图谱（结构图/光谱/机理图上传与画廊、SMILES 一键生成）** + **GNN 模型演进（打分页「反馈打分不合理」+ 反馈队列/文献 PDF·实验 CSV 导入/冻结底层微调/金标准验证闸门/版本激活回退对比——只修正 GNN，树模型作为不动基准）**
- **致谢**：本项目（含本次 v1.9.0 更新）由 DeepSeek 协助完成。
