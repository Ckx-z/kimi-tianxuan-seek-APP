...
- **最新版本**：**v1.0.2** → 📥 [GitHub Release 下载页](https://github.com/Ckx-z/kimi-tianxuan-seek-APP/releases/latest)
...
- **核心能力**：成膜概率预测（tree 模型内置必可用，GNN 可选增强自动降级）+ 打分理由 + OOD 三级判定 + 批量排序 + **DFT 计算（内置 xTB 引擎，二聚体结合能）** + 收藏夹 + 实验记录（草稿/时间线/附件/复盘字段/导出 Word）+ LLM 实验方案卡 + GraphRAG 方案迭代（基于用户实验数据闭环）+ 科研助手 Agent（对话式，9 工具读写闭环 + 记忆编译）
